...
- `GlobalSettingsOverlayController.java`, `PopupBrowserController.java`, `GlobalMainEncoderController.java`, and
  `DrumAutoPinController.java` own their named global interaction workflows. They depend on narrow ports instead of
...
- `NestedRhythmParameterState.java` owns structural and expression parameter snapshots;
  `NestedRhythmEncoderControls.java` owns the mode-specific encoder layout and lifecycle.
...
- `MelodicPitchPoolController.java`, `MelodicStepEncoderControls.java`, and `MelodicStepPadSurface.java` own pitch
  selection/audition, encoder pages, and pad gestures.
