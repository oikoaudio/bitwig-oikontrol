...
- Multiclip Seq now launches child-only scenes without changing its editing source, uses `ALT + scene` or `MUTE_1 + scene` for explicit selection, and pastes either the active Lane Clip or an exact child-only scene snapshot with `MUTE_3`.
