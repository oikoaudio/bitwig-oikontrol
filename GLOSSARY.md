...
**Baked Note Variation**: A session-controlled Akai Fire operation that writes stable, clip-wide differences into continuous per-note values. Unlike Bitwig's live velocity spread, baked values do not regenerate on each clip loop.

**Scatter**: The first Baked Note Variation policy. It uses seeded triangular sampling biased toward the mode's insertion baseline, keeps same-onset voices coherent, and writes the sampled values once.
