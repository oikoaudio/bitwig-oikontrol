...
public final class CallResponseGenerator implements MelodicGenerator {
    ...
    public MelodicPattern generate(
        ...
        final ResponseTransform transform =
                subtypeIndex < 0
                        ? ResponseTransform.values()[
                                random.nextInt(ResponseTransform.values().length)]
        ...
        for (int i = 0; i < MelodicPattern.MAX_STEPS; i++) {
            ...
            final int octaveOffset =
                    responseSide && random.nextDouble() < parameters.octaveActivity() * 0.16
                            ? 1
            ...
            final boolean slide =
                    !anchor
                            ...
                            && active[i + 1]
                            && random.nextDouble()
                                    < parameters.tension() * 0.05 + parameters.legato() * 0.14;
            final double gate =
                    slide
                            ? 0.94 + parameters.legato() * 0.16
                            : (responseSide ? 0.76 : 0.84)
                                    + parameters.density() * 0.10
                                    + parameters.legato() * (responseSide ? 0.12 : 0.06);
        ...
        for (int callIndex = 0; callIndex < half && half + callIndex < loopSteps; callIndex++) {
            ...
            if (!active[callIndex]) {
                if (responseIndex != loopSteps - 1 && random.nextDouble() < tension * 0.08) {
                    ...
                    degrees[responseIndex] =
                            clampDegree(
                                    degrees[Math.max(0, responseIndex - 1)]
                                            + (random.nextBoolean() ? 1 : -1));
                }
                continue;
            }
            final boolean keepHit =
                    callIndex == 0
                            || callIndex == half - 1
    ...
    private int chooseCallDelta(final double tension, final Random random) {
        final int[] options =
                tension >= 0.45 ? new int[] {-3, -2, -1, 1, 2, 3} : new int[] {-2, -1, 1, 2};
        return options[random.nextInt(options.length)];
    }

    private int transformDegree(
            final int callDegree,
            final int callIndex,
            final int half,
            final ResponseTransform transform,
            final Random random) {
    ...
    private void ensureResponseContrast(
            final boolean[] active,
            final int[] degrees,
            final int half,
            final int loopSteps,
        ...
        if (sameHalfContour(active, degrees, half, loopSteps)) {
            for (int i = half; i < loopSteps - 1; i++) {
                ...
                degrees[i] =
                        clampDegree(
                                degrees[i] + (transform == ResponseTransform.DOWN_ANSWER ? -1 : 1));
                return;
            }
        }
    }

    private boolean sameHalfContour(
    ...
    private int countActive(
            final boolean[] active, final int startInclusive, final int endExclusive) {
        int count = 0;
        for (int i = startInclusive; i < endExclusive; i++) {
            if (active[i]) {
                count++;
