...
public record FugueLineSettings(
        FugueDirection direction,
        FugueSpeed speed,
        int startOffset,
        int pitchDegreeOffset,
        int pitchSemitoneOffset,
        int velocityOffset,
        int chancePercent,
        int gatePercent) {
    public static FugueLineSettings init() {
        return new FugueLineSettings(
                FugueDirection.FORWARD, FugueSpeed.NORMAL, 0, 0, 0, 0, 100, 100);
    }

    public static FugueLineSettings semitone(
            final FugueDirection direction,
            final FugueSpeed speed,
            final int startOffset,
            final int pitchSemitoneOffset) {
        return new FugueLineSettings(
                direction, speed, startOffset, 0, pitchSemitoneOffset, 0, 100, 100);
    }
    ...
    public String summary() {
        return "%s %s st%d deg%+d sem%+d vel%+d ch%d gate%d"
                .formatted(
                        direction.label(),
                        speed.label(),
                        startOffset,
