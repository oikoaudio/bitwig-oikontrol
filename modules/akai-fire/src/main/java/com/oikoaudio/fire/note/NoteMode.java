...
public class NoteMode extends Layer implements StepSequencerHost {
    ...
    private void handleOikordStepPadPress(final int padIndex, final boolean pressed) {
        ...
        if (pressed) {
            ...
            } else if (clipNotesByStep.containsKey(stepIndex)) {
    ...
    private boolean extendHeldOikordRange(final int anchorStepIndex, final int targetStepIndex) {
        ...
        if (anchorStepIndex == startStep) {
            ...
            if (clipNotesByStep.containsKey(anchorStepIndex) || addedStepPads.contains(anchorStepIndex)) {
        ...
        if (!anchorNotes.isEmpty()) {
            for (final NoteStep note : anchorNotes.values()) {
                ...
                noteStepClip.setStep(startStep, note.y(), (int) Math.round(note.velocity() * 127), duration);
                noteStepClip.clearStep(note.x(), note.y());
            }
            return true;
        }
        if (clipNotesByStep.containsKey(anchorStepIndex) || addedStepPads.contains(anchorStepIndex)) {
            noteStepClip.clearStepsAtX(0, anchorStepIndex);
    ...
    private void writeOikordAtStep(final int stepIndex, final int[] notes, final int velocity, final double duration) {
        ...
        for (final int midiNote : notes) {
            noteStepClip.setStep(stepIndex, midiNote, velocity, duration);
        }
    }
    ...
    private void handlePatternUp(final boolean pressed) {
        handlePitchContextButton(pressed, 1, driver.isGlobalShiftHeld());
    }
    ...
    public void bindUser2Page(final SequencEncoderHandler handler, final Layer layer, final TouchEncoder[] encoders) {
        handler.bindNoteAccess(layer, encoders[0], NoteStepAccess.DURATION);
        handler.bindNoteAccess(layer, encoders[1], NoteStepAccess.CHANCE);
        handler.bindNoteAccess(layer, encoders[2], NoteStepAccess.VELOCITY_SPREAD);
        handler.bindNoteAccess(layer, encoders[3], NoteStepAccess.REPEATS);
    }

    @Override
    public String getModeInfo(final EncoderMode mode) {
        if (mode == EncoderMode.CHANNEL) {
            return "1: Octave Root\n2: Chord Root\n3: Chord Family\n4: Interpret";
        }
        if (mode == EncoderMode.USER_2) {
            return "1: Note Length\n2: Chance\n3: Vel Spread\n4: Repeat";
        }
