...
public final class EuclideanPhraseGenerator implements MelodicGenerator {
    ...
    public MelodicPattern generate(
        ...
        for (int i = 0; i < MelodicPattern.MAX_STEPS; i++) {
            ...
            final int degree =
                    DEGREE_POOL[(phraseIndex + random.nextInt(degreeLimit)) % DEGREE_POOL.length];
            final int octaveOffset =
                    random.nextDouble() < parameters.octaveActivity() * 0.25
                            ? (phraseIndex % 2 == 0 ? 1 : -1)
                            : 0;
            ...
            final double gate =
                    slide ? 0.96 + parameters.legato() * 0.18 : 0.7 + parameters.density() * 0.2;
