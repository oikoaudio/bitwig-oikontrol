...
/**
 * Shared refresh sequence for note-clip based modes. Rebuilds the locally tracked selected-clip
 * state, resolves which clip slot should be active, then runs the mode-specific cursor reset
 * actions.
 */
public final class NoteClipCursorRefresher {
    ...
    public static boolean refresh(
        ...
        final boolean resolved =
                ClipSlotSelectionResolver.resolve(
                        slotBank,
                        viewControlSelectedSlotIndex,
                        selectedSlotIndexSupplier.getAsInt());
