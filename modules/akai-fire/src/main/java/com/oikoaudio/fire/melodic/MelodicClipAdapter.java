...
public final class MelodicClipAdapter {
    ...
    public static MelodicPattern fromNoteSteps(
            final Map<Integer, Map<Integer, NoteStep>> noteStepsByPosition,
            final int loopSteps,
        ...
        for (int i = 0; i < MelodicPattern.MAX_STEPS; i++) {
            ...
            steps.set(
                    ...
                    new MelodicPattern.Step(
                            ...
                            note.recurrenceMask()));
            for (int offset = 1;
                    offset <= tiedSteps && i + offset < MelodicPattern.MAX_STEPS;
                    offset++) {
                steps.set(
                        i + offset, MelodicPattern.Step.rest(i + offset).withTieFromPrevious(true));
            }
        }
        return new MelodicPattern(steps, loopSteps);
    }

    public static void writeToClip(
