...
public final class BitwigEditorToolActions {
    ...
    static final String MOVE_TIME_SELECTION_TO_FIRST_ITEM_LABEL = "Move Time Selection to First Item";
    private static final String[] FOCUS_TRACK_HEADER_LABELS = {
            "Focus Track Header",
            "Focus Selected Track Header",
            "Focus/toggle Track Header"
    };
    ...
    public static void logCandidateActions(final Application application, final Consumer<String> logger) {
        ...
        logger.accept("Bitwig editor action scan: candidates containing step/input/pointer/time/selection/first/focus/toggle/clip/editor/panel");
        for (final Action action : actions) {
            if (action != null && isLikelyEditorAction(action)) {
                logger.accept("  id='%s' name='%s' menu='%s'".formatted(
                        action.getId(), action.getName(), action.getMenuItemText()));
            }
        }
    }

    private static boolean isLikelyEditorAction(final Action action) {
        final String combined = normalize("%s %s %s".formatted(action.getId(), action.getName(), action.getMenuItemText()));
        return Stream.of("step", "input", "pointer", "time", "selection", "first",
                        "focus", "toggle", "clip", "editor", "panel")
