...
final class GlobalSettingsOverlayController {
    ...
    GlobalSettingsOverlayController(
            final Layers layers,
    ...
    private void bind(
        ...
        for (int padIndex = 0; padIndex < pads.length; padIndex++) {
            ...
            pads[padIndex].bindPressed(
                    layer, pressed -> handlePad(currentPad, pressed), () -> padState(currentPad));
        }
    ...
    void refreshState() {
        final boolean momentaryComboHeld =
                host.browserButtonPressed()
                        && host.shiftHeld()
                        && !host.altHeld()
    ...
    boolean dismissForStepButton() {
        ...
        state.closeLatch();
        final boolean alreadyInStepFamily =
                switch (host.activeMode()) {
    ...
    private void showOverview() {
            ...
            oled.detailInfo(
                    ...
                    "Page: %s\n1: Pin Track %s\n2: Pin Device %s\n3: Pin Clip %s\n4: --"
                            .formatted(
                                    ...
                                    pinOverviewLabel(
                                            viewControl.getSelectedDevice().isPinned().get(),
                                            viewControl.getSelectedDevice().exists().get()),
                                    pinOverviewLabel(
                                            viewControl.getSelectedClip().isPinned().get(),
        ...
        if (state.page() == EncoderMode.USER_2) {
            oled.detailInfo(
                    ...
                    "Page: %s\n1: Create %s\n2: Record %s\n3: --\n4: --"
                            .formatted(
                                    pageLabel(),
                                    preferences.defaultClipLength(),
                                    preferences.launcherRecordLength()));
            return;
        }
        if (state.page() == EncoderMode.MIXER) {
            oled.detailInfo(
                    ...
                    "Page: %s\n1: Vel Sens %d%%\n2: Vel Ctr %d\n3: Pad Bright %s\n4: Pad Sat %s"
                            .formatted(
                                    pageLabel(),
                                    velocitySettings.sensitivity(),
                                    velocitySettings.centerVelocity(),
                                    brightnessLabel(),
                                    saturationLabel()));
            return;
        }
        oled.detailInfo(
                ...
                "Page: %s\n1: Root %s\n2: Scale %s\n3: Oct %d\n4: Note OLED %s\nTracks: %s"
                        .formatted(
                                pageLabel(),
                                NoteGridLayout.noteName(pitchContext.getRootNote()),
                                pitchContext.getScaleDisplayName(),
                                pitchContext.getOctave(),
                                preferences.noteChordDisplay(),
    ...
    private void adjust(final int encoderIndex, final int inc) {
        ...
        switch (encoderIndex) {
            case 0 -> {
                pitchContext.setRootNote(
                        Math.max(0, Math.min(11, pitchContext.getRootNote() + inc)));
        ...
        return switch (encoderIndex) {
            ...
            case 1 ->
                    reset(
                            ...
                            "No reset",
                            () ->
                                    pitchContext.setScaleIndex(
                                            pitchContext.resolveDefaultScaleIndex(
            ...
            case 2 ->
                    reset(
                            ...
                            () ->
                                    oled.valueInfo(
                                            "Octave", Integer.toString(pitchContext.getOctave())));
            case 3 ->
                    reset(
                            true,
                            "Note OLED",
                            "No reset",
                            () ->
                                    preferences.setNoteChordDisplay(
        ...
        return switch (encoderIndex) {
            case 0 ->
                    reset(
                            ...
                            "No reset",
                            () ->
                                    velocitySettings.setSensitivity(
                                            preferences.defaultVelocitySensitivity()),
                            () ->
                                    oled.valueInfo(
                                            "Velocity Sens", velocitySettings.sensitivity() + "%"));
            case 1 ->
                    reset(
                            true,
                            "Velocity Ctr",
                            ...
                            () ->
                                    oled.valueInfo(
                                            "Velocity Ctr",
                                            Integer.toString(velocitySettings.centerVelocity())));
            case 2 ->
                    reset(
                            true,
                            "Pad Bright",
                            "No reset",
                            () ->
                                    preferences.setPadBrightness(
                                            FireControlPreferences.PAD_BRIGHTNESS_DEFAULT),
                            () ->
                                    oled.valueInfo(
                                            "Pad Bright",
                                            brightnessLabel(
                                                    FireControlPreferences
                                                            .PAD_BRIGHTNESS_DEFAULT)));
            case 3 ->
                    reset(
                            true,
                            "Pad Sat",
        ...
        return switch (encoderIndex) {
            case 0 ->
                    reset(
                            ...
                            "No reset",
                            () ->
                                    preferences.setDefaultClipLength(
    ...
    private boolean reset(
        ...
        return ParameterEncoderBinding.handleExplicitResetTouch(
                true,
                host.explicitResetControl(),
                resettable,
                label,
                unavailable,
                resetAction,
            ...
            case 1 -> {
                if (velocitySettings.adjustCenterVelocity(inc)) {
                    oled.paramInfo(
                            ...
                            velocitySettings.centerVelocity(),
                            "Global Input",
                            velocitySettings.minCenterVelocity(),
                            velocitySettings.maxCenterVelocity());
                }
            }
    ...
    private void adjustPins(final int encoderIndex, final int inc) {
        switch (encoderIndex) {
            case 0 ->
                    applyPinEncoder(
                            "Pin Track", viewControl.getCursorTrack().isPinned(), true, inc);
            case 1 ->
                    applyPinEncoder(
                            "Pin Device",
                            viewControl.getSelectedDevice().isPinned(),
                            viewControl.getSelectedDevice().exists().get(),
                            inc);
            case 2 ->
                    applyPinEncoder(
                            "Pin Clip",
                            viewControl.getSelectedClip().isPinned(),
                            viewControl.getSelectedClip().exists().get(),
                            inc);
            default -> showOverview();
        }
    ...
    private void showInput(final int encoderIndex) {
        switch (encoderIndex) {
            ...
            case 1 ->
                    oled.valueInfo(
                            "Velocity Center", Integer.toString(velocitySettings.centerVelocity()));
    ...
    private void adjustBrightness(final int inc) {
        final double next =
                FireControlPreferences.normalizePadBrightness(
                        preferences.padBrightness()
    ...
    private void adjustSaturation(final int inc) {
        final double next =
                FireControlPreferences.normalizePadSaturation(
                        preferences.padSaturation()
    ...
    private void adjustLauncherRecordLength(final int inc) {
        final int nextIndex =
                steppedIndex(
                        ...
                        inc);
        preferences.setLauncherRecordLength(
                FireControlPreferences.LAUNCHER_RECORD_LENGTHS[nextIndex]);
