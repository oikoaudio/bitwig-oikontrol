...
public class BiColorButton {
    ...
    public BiColorButton(
        ...
        hwButton.pressedAction()
                .setPressureActionMatcher(
                        midiIn.createNoteOnVelocityValueMatcher(0, assignment.getNoteValue()));
        hwButton.releasedAction()
                .setActionMatcher(midiIn.createNoteOffActionMatcher(0, assignment.getNoteValue()));
        hwButton.isPressed().markInterested();
        light =
                driver.getSurface()
                        .createMultiStateHardwareLight("BUTTON_LIGHT_" + assignment.toString());
    ...
    public void bindPressed(
            final Layer layer,
            final Consumer<Boolean> target,
            final BiColorLightState onColor,
    ...
    public void bindPressed(
            final Layer layer, final Consumer<Boolean> target, final BiColorLightState onColor) {
        layer.bind(hwButton, hwButton.pressedAction(), () -> target.accept(true));
        layer.bind(hwButton, hwButton.releasedAction(), () -> target.accept(false));
        layer.bindLightState(
                () -> hwButton.isPressed().get() ? onColor : BiColorLightState.OFF, light);
    }
