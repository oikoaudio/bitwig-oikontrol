 ...
 * occupied steps, accent emphasis, and playhead highlighting so the modes do not duplicate RGB
 * decisions.
 */
public final class StepPadLightHelper {
    ...
    private static final RgbLightState CLIP_START_INDICATOR = new RgbLightState(30, 0, 127, true);

    private StepPadLightHelper() {}

    public static RgbLightState renderEmptyStep(final int stepIndex, final int playingStep) {
        if (stepIndex == playingStep) {
            return RgbLightState.WHITE;
        }
        return (stepIndex / 4) % 2 == 0 ? RgbLightState.GRAY_1 : RgbLightState.GRAY_2;
    }
    ...
    public static RgbLightState renderOccupiedStep(
            final RgbLightState base, final boolean accented, final boolean playing) {
        if (accented) {
    ...
    public static RgbLightState renderPlayheadHighlight(final RgbLightState base) {
        final RgbLightState brightest = base.getBrightest();
    ...
    public static RgbLightState renderClipStartIndicator(final RgbLightState base) {
        return CLIP_START_INDICATOR;
    }

    public static RgbLightState renderClipStartColumnOverlay(
            final int padColumn, final int shiftedClipStartColumn, final RgbLightState base) {
    ...
    public static int nearestVisibleStepForShiftedClipStart(
        ...
        final int totalSteps =
                Math.max(1, (int) Math.round(Math.max(stepLength, loopLength) / stepLength));
    ...
    public static int nearestBucketForShiftedClipStart(
            final double playStart, final double loopLength, final int bucketCount) {
        if (bucketCount <= 0) {
            return -1;
        }
        final double wrapped = shiftedPlayStart(playStart, loopLength);
    ...
    private static double shiftedPlayStart(final double playStart, final double loopLength) {
        ...
        return normalized > POSITION_EPSILON && length - normalized > POSITION_EPSILON
                ? normalized
                : -1.0;
    }
}
