...
final class LauncherRetriggerActions {
     ...
     *
     * @return true when Bitwig accepted a global retrigger action, false when callers should fall back.
     */
    ...
    /**
     * Fallback for older or differently named Bitwig actions: retrigger only the selected launcher clip.
     */
    ...
    /**
     * Resolve the global action defensively because Bitwig actions are string-addressed and their ids are
     * less stable than typed API methods. Try the expected id first, then the visible GUI label, then scan
     * all exposed actions for a matching id/name/menu label.
     */
