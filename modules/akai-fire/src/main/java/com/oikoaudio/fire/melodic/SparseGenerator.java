...
public final class SparseGenerator implements MelodicGenerator {
    ...
    public MelodicPattern generate(
        ...
        for (final int position : SPARSE_POSITIONS) {
            ...
            if (position != 0
                    && position != loopSteps - 1
                    && random.nextDouble() > 0.7 + parameters.density() * 0.2) {
                continue;
            }
            final int degree =
                    SPARSE_DEGREES[
                            random.nextInt(
                                    Math.max(1, 1 + (int) Math.round(parameters.tension() * 2)))];
            final int octaveOffset =
                    random.nextDouble() < parameters.octaveActivity() * 0.2 ? 1 : 0;
