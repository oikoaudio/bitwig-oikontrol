...
public class RgbButton {
    ...
    public RgbButton(final int pad, final AkaiFireOikontrolExtension driver) {
        ...
        hwButton.pressedAction()
                .setPressureActionMatcher(midiIn.createNoteOnVelocityValueMatcher(0, 0x36 + index));
        hwButton.releasedAction()
                .setActionMatcher(midiIn.createNoteOffActionMatcher(0, 0x36 + index));
    ...
    public void bindPressedVelocity(
            final Layer layer,
            final IntConsumer pressedVelocityTarget,
        ...
        layer.bind(
                ...
                hwButton.pressedAction(),
                pressure ->
                        pressedVelocityTarget.accept(
                                Math.max(1, Math.min(127, (int) Math.round(pressure * 127.0)))));
