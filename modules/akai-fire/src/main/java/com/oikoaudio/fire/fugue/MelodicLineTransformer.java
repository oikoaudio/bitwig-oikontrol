...
public final class MelodicLineTransformer {
    ...
    public static FuguePattern transform(
            final FuguePattern source,
        ...
        final int phraseSteps =
                settings.speed().isSpeedUp() ? repeatedPhraseSteps(source) : loopSteps;
        for (int sourceIndex = 0; sourceIndex < phraseSteps; sourceIndex++) {
            for (final MelodicPattern.Step sourceStep : source.notesAt(sourceIndex)) {
                if (!sourceStep.active()
                        || sourceStep.tieFromPrevious()
                        || sourceStep.pitch() == null) {
                    continue;
                }
                writeTransformedStep(
                        out,
                        sourceStep,
                        sourceIndex,
                        phraseSteps,
    ...
    private static void writeTransformedStep(
            final List<List<MelodicPattern.Step>> out,
            ...
            final MusicalScale scale,
            final int rootNote) {
        final int directionalIndex =
                directionalIndex(sourceIndex, phraseSteps, settings.direction());
        final int destination =
                transformedDestination(directionalIndex, phraseSteps, loopSteps, settings);
        if (destination < 0 || destination >= loopSteps) {
            return;
        }
        final int pitch =
                ScaleAwareTransposer.transposeDiatonicThenChromatic(
                        sourceStep.pitch(),
                        settings.pitchDegreeOffset(),
                        settings.pitchSemitoneOffset(),
                        scale,
                        rootNote);
        final MelodicPattern.Step transformed =
                transformedStep(sourceStep, destination, pitch, settings);
    ...
    private static MelodicPattern.Step transformedStep(
            final MelodicPattern.Step sourceStep,
            final int destination,
            final int pitch,
            final FugueLineSettings settings) {
        return new MelodicPattern.Step(
                destination,
                true,
                false,
                pitch,
                ...
                Math.max(
                        0.02,
                        settings.speed().scaleGate(sourceStep.gate())
                                * settings.gatePercent()
                                / 100.0),
                sourceStep.chance() * settings.chancePercent() / 100.0,
                sourceStep.accent(),
                sourceStep.slide(),
                sourceStep.recurrenceLength(),
                sourceStep.recurrenceMask());
    }

    private static int transformedDestination(
            final int directionalIndex,
            final int phraseSteps,
            final int loopSteps,
            final FugueLineSettings settings) {
        final int period =
                settings.speed().isSpeedUp()
                        ? settings.speed().transformedLoopSteps(phraseSteps)
                        : loopSteps;
        ...
        return Math.floorMod(
                settings.startOffset() + settings.speed().scaleStart(directionalIndex), period);
    }

    private static void repeatSpeedUpStep(
            final List<List<MelodicPattern.Step>> out,
            final MelodicPattern.Step transformed,
            final int loopSteps,
            final int period) {
        for (int destination = transformed.index() + period;
                destination < loopSteps;
                destination += period) {
    ...
    private static boolean sameMusicalStepList(
            final List<MelodicPattern.Step> a, final List<MelodicPattern.Step> b) {
        if (a.size() != b.size()) {
            return false;
        }
    ...
    private static boolean sameMusicalStep(
            final MelodicPattern.Step a, final MelodicPattern.Step b) {
    ...
    private static int directionalIndex(
            final int sourceIndex, final int loopSteps, final FugueDirection direction) {
        return switch (direction) {
            case FORWARD -> sourceIndex;
            case REVERSE -> loopSteps - 1 - sourceIndex;
            case PING_PONG ->
                    sourceIndex < loopSteps / 2
                            ? sourceIndex
