...
public final class PadMatrixBindings {
    ...
    public static void bindPressedVelocity(
            final Layer layer, final RgbButton[] pads, final Host host) {
        for (int index = 0; index < pads.length; index++) {
            final int padIndex = index;
            pads[index].bindPressedVelocity(
                    layer,
                    velocity -> host.handlePadPress(padIndex, true, velocity),
                    () -> host.handlePadPress(padIndex, false, 0),
                    () -> host.padLight(padIndex));
        }
    }

    public static void bindPressed(
            final Layer layer, final RgbButton[] pads, final PressHost host) {
    ...
    public static void bindPressed(
            ...
            final int count,
            final PressHost host) {
        for (int index = 0; index < pads.length; index++) {
            if (index < startIndex || index >= startIndex + count) {
                continue;
            }
            final int padIndex = index - startIndex;
            pads[index].bindPressed(
                    layer,
                    pressed -> host.handlePadPress(padIndex, pressed),
                    () -> host.padLight(padIndex));
        }
    }
