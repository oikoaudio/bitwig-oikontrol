    ...
    public MelodicPattern generate(
        ...
        for (int i = 0; i < MelodicPattern.MAX_STEPS; i++) {
            ...
            final boolean active =
                    i % 2 == 0 || random.nextDouble() < 0.35 + parameters.density() * 0.25;
            if (!active) {
                steps.add(MelodicPattern.Step.rest(i));
                continue;
            }
            final boolean highOctave =
                    i % 4 == 2 || random.nextDouble() < 0.25 + parameters.octaveActivity() * 0.45;
