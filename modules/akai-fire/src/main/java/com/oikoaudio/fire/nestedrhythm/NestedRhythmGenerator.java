...
public final class NestedRhythmGenerator {
    ...
    public NestedRhythmPattern generate(final Settings settings) {
        ...
        final List<PulseSpec> retainedBar = retainedBarStructure(normalized, barFineSteps);

        final List<NestedRhythmPattern.PulseEvent> events = new ArrayList<>(retainedBar.size() * normalized.barCount());
        final List<Integer> rawVelocities = new ArrayList<>(events.size());
        int order = 0;
        for (int barIndex = 0; barIndex < normalized.barCount(); barIndex++) {
            final int barOffset = barIndex * barFineSteps;
            for (final PulseSpec pulse : retainedBar) {
                final int fineStart = pulse.fineStart() + barOffset;
    ...
    private List<PulseSpec> retainedBarStructure(final Settings settings, final int barFineSteps) {
        final TreeMap<Integer, PulseSpec> structure = new TreeMap<>();
        addAnchors(structure, settings, barFineSteps);
        applyTuplet(structure, settings, barFineSteps);
        applyRatchet(structure, settings, barFineSteps);
    ...
    private void addAnchors(final TreeMap<Integer, PulseSpec> structure,
        ...
        for (int beat = 0; beat < settings.meterNumerator(); beat++) {
            final int fineStart = Math.min(beat * beatFineSteps, barFineSteps - 1);
    ...
    private void applyTuplet(final TreeMap<Integer, PulseSpec> structure,
        ...
        if (settings.tupletCoverage() == TupletCoverage.BACK_HALF) {
            length = Math.max(1, barFineSteps / 2);
            start = settings.tupletPhase() == 0 ? barFineSteps - length : 0;
        } else {
            start = 0;
            length = barFineSteps;
        }
    ...
    private void applyRatchet(final TreeMap<Integer, PulseSpec> structure,
        ...
        for (final int beat : ratchetedBeats(settings)) {
            final int start = Math.min(beat * beatFineSteps, barFineSteps - 1);
            structure.subMap(start, true, Math.min(barFineSteps - 1, start + beatFineSteps - 1), true).clear();
    ...
    private List<Integer> ratchetedBeats(final Settings settings) {
        ...
        final int width = Math.max(1, Math.min(beatCount, settings.ratchetWidth()));
        final int phase = Math.floorMod(settings.ratchetPhase(), beatCount);
        final List<Integer> beats = new ArrayList<>(width);
        for (int index = 0; index < width; index++) {
            beats.add(ratchetRegionAt(beatCount, index + phase));
        }
        return beats;
    }
    ...
    public record Settings(int midiNote, double density, int tupletCount, TupletCoverage tupletCoverage,
        ...
        public Settings normalized() {
            return new Settings(
                    ...
                    Math.max(1, Math.min(normalizeNumerator(meterNumerator), ratchetWidth)),
                    Math.floorMod(ratchetPhase, Math.max(1, normalizeNumerator(meterNumerator))),
                    ...
                    normalizeNumerator(meterNumerator),
                    normalizeDenominator(meterDenominator),
                    Math.max(1, Math.min(MAX_BARS, barCount)));
        }
