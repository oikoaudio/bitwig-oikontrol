...
public class OledDisplay {
    ...
    private final byte[] oledCmd =
            new byte[] {SE_ST, MAN_ID_AKAI, DEVICE_ID, PRODUCT_ID, SE_OLED_RGB, 00};
    ...
    public boolean hasRecentTransientMessage(final long quietPeriodMs) {
        return transientMessageStartedAtMs > 0
                && System.currentTimeMillis() - transientMessageStartedAtMs
                        < Math.max(0, quietPeriodMs);
    }
    ...
    public void setFooterLegendPosition(final EncoderLegendPosition footerLegendPosition) {
        final EncoderLegendPosition normalized =
                footerLegendPosition == null ? EncoderLegendPosition.BOTTOM : footerLegendPosition;
    ...
    private void sendImageData(final int[] imageData, final int startPage, final int endPage) {
        ...
        for (int page = Math.max(0, startPage);
                page <= Math.min(OLED_PAGE_COUNT - 1, endPage);
                page++) {
    ...
    public void showLine(final Line line) {
        sendContentString(
                line.getSize(), line.getJustification(), line.getOffset(), line.getViewText());
    }
    ...
    public void valueInfoWithBarNoClear(
            final String title,
            final String value,
            final double normalizedValue,
            final boolean biPolar) {
    ...
    private int lastImagePageWithFooter() {
        return footerLegendPosition == EncoderLegendPosition.TOP
                ? OLED_PAGE_COUNT - 1
                : OLED_PAGE_COUNT - 2;
    }
    ...
    private void sendContentString(
            final int fontSize,
            final TextJustification justification,
            final int placement,
            final String text) {
    ...
    private void sendString(
            ...
            final int placement,
            final String text,
            final boolean clearGraphics,
            final boolean invalidateAllImages) {
