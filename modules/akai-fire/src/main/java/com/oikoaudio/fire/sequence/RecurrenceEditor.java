...
/**
 * Temporary editing layer for note recurrence masks in drum sequencing mode. It owns the pad
 * overlay used while one or more held steps are being edited.
 *
 * <p>This is a legacy encoder-entered editor path. The current direct recurrence workflow for
 * step/pulse modes is the hold-step recurrence row backed by {@link RecurrencePadInteraction}; Drum
 * XOX no longer exposes a recurrence encoder in its encoder banks. Keep this class only while older
 * entry points or compatibility needs remain.
 */
public class RecurrenceEditor {
    ...
    public RecurrenceEditor(
            final AkaiFireOikontrolExtension driver, final DrumSequenceMode parent) {
    ...
    private void initClipControlButtons(
            final Layer clipLayer, final AkaiFireOikontrolExtension driver) {
    ...
    private RgbLightState getState(final int index, final int mask) {
        ...
        return recurrencePads.padLight(
                index, RecurrencePattern.of(currentLen, currentMask), RgbLightState.PURPLE);
    }
    ...
    private void handleMask(final boolean pressed, final int index) {
        ...
        recurrencePads.handlePadPress(
                index, pressed, hasTarget, recurrence, () -> {}, this::toggleMask, this::applySpan);
    }
    ...
    private void applySpan(final int span) {
        ...
        final RecurrencePattern updated =
                RecurrencePattern.of(currentLen, currentMask).applySpanGesture(span);
