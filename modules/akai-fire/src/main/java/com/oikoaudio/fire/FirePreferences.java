...
/**
 * Owns the Bitwig preference handles used by the Akai Fire extension and exposes normalized, typed
 * access.
 */
public final class FirePreferences {
    ...
    public FirePreferences(final Preferences preferences, final Listener listener) {
        ...
        launchQuantization.addValueObserver(
                value ->
                        listener.launchQuantizationChanged(
                                FireControlPreferences.toLaunchQuantizationValue(value)));
        mainEncoderStartup.addValueObserver(
                value ->
                        listener.mainEncoderStartupChanged(
                                FireControlPreferences.normalizeMainEncoderStartupState(value)));
        drumPinMode.addValueObserver(
                value ->
                        listener.drumPinModeChanged(
                                FireControlPreferences.shouldAutoPinFirstDrumMachine(value)));
        padBrightnessValue.addRawValueObserver(
                value -> {
    ...
    public int defaultVelocitySensitivity() {
        return FireControlPreferences.toDefaultVelocitySensitivity(
                defaultVelocitySensitivity.get());
    }
