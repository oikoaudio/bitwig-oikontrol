...
public final class FugueClipAdapter {
    ...
    public static FuguePattern sourceFromChannelOne(
            final Map<Integer, Map<Integer, Map<Integer, NoteStep>>> noteSteps,
            final int loopSteps,
            final double stepLength) {
        return fromChannel(noteSteps, SOURCE_CHANNEL, loopSteps, stepLength);
    }

    public static FuguePattern fromChannel(
            final Map<Integer, Map<Integer, Map<Integer, NoteStep>>> noteSteps,
            final int channel,
            final int loopSteps,
            final double stepLength) {
        ...
        for (int i = 0; i < FuguePattern.MAX_STEPS; i++) {
            final Map<Integer, NoteStep> notesAtStep =
                    noteSteps.getOrDefault(channel, Map.of()).get(i);
            if (notesAtStep == null || notesAtStep.isEmpty()) {
                continue;
            }
            final List<ObservedNote> notes =
            ...
            for (final ObservedNote note : notes) {
                ...
                tiedSteps =
                        Math.max(
                                tiedSteps,
                                (int) Math.floor((duration - stepLength * 0.98) / stepLength));
                steps.get(i)
                        .add(
                                new MelodicPattern.Step(
                                        ...
                                        Math.max(0.1, duration / stepLength),
                                        note.chance(),
                                        note.velocity() >= 0.87,
                                        duration > stepLength * 1.05,
                                        note.recurrenceLength(),
                                        note.recurrenceMask()));
            }
            for (int offset = 1;
                    offset <= tiedSteps && i + offset < FuguePattern.MAX_STEPS;
                    offset++) {
                if (steps.get(i + offset).isEmpty()) {
                    steps.get(i + offset)
                            .add(MelodicPattern.Step.rest(i + offset).withTieFromPrevious(true));
                }
            }
        }
    ...
    public static void writeDerivedLine(
    ...
    private static void applyGeneratedNoteExpression(
            final NoteStep noteStep, final MelodicPattern.Step step) {
        try {
    ...
    public static void duplicateChannelRange(
            final PinnableCursorClip clip,
            final Map<Integer, Map<Integer, Map<Integer, NoteStep>>> observedSteps,
            final int channel,
        ...
        final Map<Integer, Map<Integer, NoteStep>> channelSteps =
                observedSteps.getOrDefault(channel, Map.of());
        for (final Map.Entry<Integer, Map<Integer, NoteStep>> stepEntry : channelSteps.entrySet()) {
            ...
            for (final NoteStep note : stepEntry.getValue().values()) {
                ...
                clip.setStep(
                        ...
                        targetX,
                        snapshot.pitch(),
                        (int) Math.round(snapshot.velocity() * 127.0),
                        snapshot.duration());
            }
        }
    }
