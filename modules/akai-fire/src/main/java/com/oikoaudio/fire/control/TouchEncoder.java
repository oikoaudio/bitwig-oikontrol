...
public class TouchEncoder {
    ...
    public TouchEncoder(
            final int controlId, final int buttonId, final AkaiFireOikontrolExtension driver) {
        ...
        encoder.setAdjustValueMatcher(
                midiIn.createRelative2sComplementCCValueMatcher(0, noteValue, 127));
        ...
        touchButton
                .pressedAction()
                .setPressureActionMatcher(midiIn.createNoteOnVelocityValueMatcher(0, buttonId));
        touchButton
                .releasedAction()
                .setActionMatcher(midiIn.createNoteOffActionMatcher(0, buttonId));
    }
    ...
    public void bindEncoderBehavior(
            final Layer layer,
        ...
        layer.bind(
                ...
                createRelativeMagnitudeBinder(
                        rawUnits -> {
                            ...
                            final int inc =
                                    RelativeEncoderMagnitude.toStandardTurnStep(rawUnits, fine);
    ...
    public void bindContinuousEncoder(
            final Layer layer, final BooleanSupplier fineSupplier, final IntConsumer action) {
    ...
    public void bindThresholdedEncoder(
            final Layer layer,
            final int normalThreshold,
            final int fineThreshold,
            final BooleanSupplier fineSupplier,
            final IntConsumer action) {
        bindEncoderBehavior(
                layer,
                fineSupplier,
                EncoderTurnBehavior.quantizedSteps(normalThreshold, fineThreshold),
                action);
    }
    ...
    public void bindRelativeMagnitudeEncoderBehavior(
            final Layer layer,
            final BooleanSupplier fineSupplier,
            final EncoderTurnBehavior behavior,
            final IntConsumer action) {
    ...
    public RelativeHardwarControlBindable createIncrementBinder(final IntConsumer consumer) {
        return createRelativeMagnitudeBinder(
                rawUnits -> {
                    final int inc = RelativeEncoderMagnitude.toDirectionStep(rawUnits);
                    if (inc != 0) {
                        consumer.accept(inc);
    ...
    public RelativeHardwarControlBindable createRelativeMagnitudeBinder(
            final IntConsumer consumer) {
        return host.createRelativeHardwareControlAdjustmentTarget(
                adjustment -> {
