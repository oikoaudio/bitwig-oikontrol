...
public class RgbLightState extends InternalHardwareLightState {
    ...
    private RgbLightState(
            final byte red, final byte green, final byte blue, final boolean variants) {
        super();
        this.red = red;
        this.green = green;
        this.blue = blue;
        if (variants) {
            softDimmed =
                    new RgbLightState(
                            scaleSoftDim(red), scaleSoftDim(green), scaleSoftDim(blue), false);
            dimmed =
                    new RgbLightState(
                            red / DIM_FACTOR, green / DIM_FACTOR, blue / DIM_FACTOR, false);
            veryDimmed =
                    new RgbLightState(
                            red / ULTRA_DIM_FACTOR,
                            green / ULTRA_DIM_FACTOR,
                            blue / ULTRA_DIM_FACTOR,
                            false);
            brightend =
                    new RgbLightState(
                            Math.min(red + BRIGHT_FACTOR, 127),
                            Math.min(green + BRIGHT_FACTOR, 127),
                            Math.min(blue + BRIGHT_FACTOR, 127),
                            false);
            brightest =
                    new RgbLightState(
                            Math.min(red + MAX_BRIGHT_FACTOR, 127),
                            Math.min(green + MAX_BRIGHT_FACTOR, 127),
                            Math.min(blue + MAX_BRIGHT_FACTOR, 127),
                            false);
        }
    }
