...
class NotePlayControllerTest {
    ...
    void delegatesLifecycleAndPadPressToLiveCollaborators() {
        ...
        final NotePlayController mode =
                new NotePlayController(
                        new NoteLiveControlSurface(
                                ...
                                new EncoderTouchDisplayHandler(() -> {}),
                                (title, detail) -> {},
                                (title, detail) -> {},
                                () -> {}),
                        new NoteLivePadPerformer(
                                ...
                                pad -> new int[] {60 + pad},
                                (baseVelocity, rawVelocity) -> baseVelocity));
        ...
        assertEquals(
                List.of(
                        ...
                        "sostenuto:0"),
                events.stream()
                        .filter(s -> !s.startsWith("deactivate:") && !s.startsWith("activate:"))
                        .toList());
    }

    private static NoteLiveEncoderModeControls.LayerHandle layer(
            final List<String> events, final String name) {
