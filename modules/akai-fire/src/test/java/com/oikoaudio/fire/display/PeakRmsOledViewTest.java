...
class PeakRmsOledViewTest {
    ...
    void drawsPeakRmsRowsWithBottomLegend() {
        ...
        order.verify(oled)
                .sendString(0, OledDisplay.TextJustification.LEFT, 0, "Peak          RMS");
        order.verify(oled)
                .sendString(0, OledDisplay.TextJustification.LEFT, 7, EncoderFooterLegend.MIXER);
    ...
    void drawsPeakRmsRowsWithTopLegend() {
        ...
        order.verify(oled).clearScreen();
        order.verify(oled)
                .sendString(0, OledDisplay.TextJustification.LEFT, 0, EncoderFooterLegend.MIXER);
        order.verify(oled)
                .sendString(0, OledDisplay.TextJustification.LEFT, 2, "Peak          RMS");
    ...
    void transientValuePreservesBottomLegendRows() {
        ...
        order.verify(oled).sendString(2, OledDisplay.TextJustification.LEFT, 4, "-16.1  -inf");
        order.verify(oled)
                .sendString(2, OledDisplay.TextJustification.LEFT, 0, "                    ");
        order.verify(oled)
                .sendString(2, OledDisplay.TextJustification.LEFT, 1, "                    ");
        order.verify(oled)
                .sendString(3, OledDisplay.TextJustification.LEFT, 2, "                    ");
        order.verify(oled)
                .sendString(2, OledDisplay.TextJustification.LEFT, 4, "                    ");
        for (int row = 0; row < 7; row++) {
            order.verify(oled)
                    .sendString(0, OledDisplay.TextJustification.LEFT, row, "                    ");
        }
        order.verify(oled)
                .sendString(0, OledDisplay.TextJustification.LEFT, 7, EncoderFooterLegend.MIXER);
