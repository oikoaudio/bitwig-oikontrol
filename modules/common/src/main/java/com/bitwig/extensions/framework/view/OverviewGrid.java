    ...
    public void setUpFocusScene(final TrackBank mainBank) {
        ...
        mainBank.sceneBank()
                ...
                .addValueObserver(
                        pos -> {
                            ...
                            if (maxTrackBank.sceneBank().scrollPosition().get()
                                    != getSceneOffset()) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < size; j++) {
                ...
                slot.isPlaybackQueued()
                        .addValueObserver(isQueued -> markSceneQueued(sceneIndex, isQueued));
                slot.isPlaying()
                        .addValueObserver(isQueued -> markScenePlaying(sceneIndex, isQueued));
            }
        }
    }
