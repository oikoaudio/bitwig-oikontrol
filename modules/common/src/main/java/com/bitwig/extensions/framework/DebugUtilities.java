...
public class DebugUtilities {
    public static Layer createDebugLayer(
            final Layers layers, final HardwareSurface hardwareSurface) {
    ...
    public static void addDebugBindings(final Layer layer, final HardwareSurface hardwareSurface) {
        ...
        for (final HardwareControl control : hardwareSurface.hardwareControls()) {
            ...
            if (!control.getLabel().isEmpty()) controlName += " " + control.getLabel();

            final String prefix = controlName + ": ";

            layer.bind(
                    control,
                    control.beginTouchAction(),
                    () -> host.println(prefix + "begin touch"));
            ...
            if (control instanceof HardwareButton) {
                ...
                layer.bindPressed(
                        button,
                        pressure -> host.println(prefix + "pressed with pressure " + pressure));
                layer.bindReleased(
                        button,
