...
public class Context {
    ...
    /**
     * Creates the dependency injection context from the controller extension. Creates and registers
     * the following bitwig elements as services:
     *
     * <ul>
     *   <li>{@link Transport}
     *   <li>{@link ControllerHost}
     *   <li>{@link Application}
     *   <li>{@link HardwareSurface}
     *   <li>a layers object {@link Layers}
     * </ul>
     *
     * The package of the extension and all sub-packages are scanned for other classes annotated
     * with the {@link Component} annotation. Another way to create services is to create them via
     * the {@link Context#create(Class)} method. This instantiates the component and registers it as
     * Service in the context. The service is access by the class of the component. Further more
     * objects can be registered as services via {@link Context#create(Class)} or {@link
     * Context#registerService(Class, Object)}
     *
    ...
    public void activate() {
        invoke(Activate.class);
    }

    public void deactivate() {
        invoke(Deactivate.class);
    }

    private <T extends Annotation> void invoke(final Class<T> annotation) {
        final HashSet<Object> serviced = new HashSet<>();
        final Collection<Object> components = services.values();
        for (final Object component : components) {
            if (!serviced.contains(component) && invokeActivation(component, annotation)) {
                serviced.add(component);
            }
        }
    }

    private <T extends Annotation> boolean invokeActivation(
            final Object comp, final Class<T> annotation) {
        final Class<?> clazz = comp.getClass();
        final Method[] methods = clazz.getMethods();
        for (final Method method : methods) {
            final T activateAnnotation = method.getAnnotation(annotation);
            if (activateAnnotation != null && method.getParameterCount() == 0) {
                try {
                    ...
                    throw new DiException(
                            "failed to activate component " + clazz.getName(), exception);
                }
            }
        }
        return false;
    ...
    private record ComponentClassBind(Class<?> clazz, Component component)
            implements Comparable<ComponentClassBind> {
        //
    ...
    private void scanPackage(final Class<?> baseClass, final Package... packages) {
        try {
            ...
            final List<ComponentClassBind> components =
                    classes.stream() //
    ...
    private <T> void create(final ComponentClosure<T> closure) {
        ...
        try {
            ...
            if (lastConstructor == null) {
                throw new DiException(
                        String.format(
                                "Class %s has not available constructor",
                                closure.clazz.getSimpleName()));
            }
    ...
    private void tryToCompleteIncompleteSetter(final Class<?> newType) {
        ...
        while (it.hasNext()) {
            final ComponentClosure<?> closure = it.next();
            if (closure.instance != null
                    && closure.missingSetters.remove(newType)
    ...
    private void injectBySetter(final Object object, final ComponentClosure<?> closure)
    ...
    private <T> void invokePostConstruct(final T object, final Method method)
            throws IllegalAccessException, InvocationTargetException {
    ...
    private <T> Object[] getConstructableArguments(
            final Constructor<T> constructor, final Component compAnnotation) {
        final Parameter[] parameterTypes = constructor.getParameters();
    ...
    private <T> boolean inject(final T object, final Field field)
            throws IllegalArgumentException, IllegalAccessException {
        @SuppressWarnings("unchecked")
        final T serviceObject = (T) getServiceImpl(field.getType());
