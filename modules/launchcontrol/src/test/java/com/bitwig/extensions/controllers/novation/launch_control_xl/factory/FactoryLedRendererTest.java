...
class FactoryLedRendererTest {
   ...
   void rendersTrackFocusAndMuteButtonsWithExistingColors()
   {
      ...
      strips[0] = FactoryUiSnapshot.Strip.existing(true, false, false,
         FactoryUiSnapshot.Value.missing(), missingValues(), missingValues(), missingValues());
      final FactoryUiSnapshot state = snapshot(FactoryUiSnapshot.Mode.SEND_2_DEVICE_1,
         FactoryUiSnapshot.TrackControl.MUTE, false, 0, 0, strips);
   ...
   {
      final FactoryUiSnapshot.Strip[] strips = emptyStrips();
      strips[0] = FactoryUiSnapshot.Strip.existing(false, false, false,
         FactoryUiSnapshot.Value.of(0.75),
         new FactoryUiSnapshot.Value[]{FactoryUiSnapshot.Value.of(0), FactoryUiSnapshot.Value.of(0.25), FactoryUiSnapshot.Value.missing()},
         new FactoryUiSnapshot.Value[]{FactoryUiSnapshot.Value.of(0.75), FactoryUiSnapshot.Value.missing(), FactoryUiSnapshot.Value.missing()},
         missingValues());
      final FactoryUiSnapshot state = snapshot(FactoryUiSnapshot.Mode.SEND_2_DEVICE_1,
         FactoryUiSnapshot.TrackControl.NONE, true, 0, 2, strips);
   ...
   {
      final FactoryUiSnapshot track = snapshot(FactoryUiSnapshot.Mode.SEND_3,
         FactoryUiSnapshot.TrackControl.SOLO, false, 0, 0, emptyStrips())
         .withNavigation(true, false, true, false, false, false);
      final FactoryUiSnapshot device = track.withDeviceOn(true)
         .withNavigation(true, false, false, false, true, true);

      assertArrayEquals(new int[]{12, 12, 62, 12, 62, 12, 62, 12},
         FactoryLedRenderer.render(track).rightButtons());
      assertArrayEquals(new int[]{62, 12, 62, 12, 62, 12, 62, 62},
   ...
   {
      return new FactoryUiSnapshot(FactoryUiSnapshot.Surface.FACTORY, mode, control, deviceOn, page, pages, 0, strips,
         missingValues(8), missingValues(8), false, false, false, false, false, false);
   }
   ...
   private static FactoryUiSnapshot.Value[] missingValues(final int count)
   {
      ...
      for (int i = 0; i < count; i++)
         values[i] = FactoryUiSnapshot.Value.missing();
      return values;
   }
}
