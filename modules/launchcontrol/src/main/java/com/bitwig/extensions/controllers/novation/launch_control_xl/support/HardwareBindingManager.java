...
public final class HardwareBindingManager {
    ...
    public void attachFactory(
            ...
            final int sendUpCc,
            final int sendDownCc,
            final int trackLeftCc,
            final int trackRightCc,
    ...
    public void attachDrum(
        ...
        bindTransportAndTrack(
                channel,
                DrumMapping.SEND_UP_CC,
                DrumMapping.SEND_DOWN_CC,
                DrumMapping.TRACK_LEFT_CC,
                DrumMapping.TRACK_RIGHT_CC);
        bindNotes(
                channel,
                trackFocusNotes,
                trackControlNotes,
    ...
    public void clear() {
        for (final AbsoluteHardwareKnob knob : this.knobs) {
            if (knob != null) knob.setAdjustValueMatcher(null);
        }
        for (final HardwareSlider slider : this.sliders) {
            if (slider != null) slider.setAdjustValueMatcher(null);
        }
        bindCC(this.sendUp, null, 0);
        bindCC(this.sendDown, null, 0);
        bindCC(this.trackLeft, null, 0);
        bindCC(this.trackRight, null, 0);
        for (final HardwareButton button : this.trackFocusButtons) bindNote(button, null, 0);
        for (final HardwareButton button : this.trackControlButtons) bindNote(button, null, 0);
        ...
        for (int i = 0; i < this.knobs.length; i++) {
            ...
            if (knob != null)
                knob.setAdjustValueMatcher(
                        this.midiIn.createAbsoluteCCValueMatcher(channel, this.knobCcNumbers[i]));
        }
    }
    ...
    private void bindKnobsDrum(final int channel) {
        for (int i = 0; i < this.knobs.length; i++) {
            ...
            if (i < 8)
                knob.setAdjustValueMatcher(
                        this.midiIn.createAbsoluteCCValueMatcher(
                                channel, DrumMapping.KNOB_ROW1_CCS[i]));
            else if (i < 16)
                knob.setAdjustValueMatcher(
                        this.midiIn.createAbsoluteCCValueMatcher(
                                channel, DrumMapping.KNOB_ROW2_CCS[i - 8]));
            else
                knob.setAdjustValueMatcher(
                        this.midiIn.createAbsoluteCCValueMatcher(
                                channel, DrumMapping.KNOB_ROW3_CCS[i - 16]));
        }
    }
    ...
    private void bindSlidersFactory(final int channel) {
        for (int i = 0; i < this.sliders.length; i++) {
            ...
            if (slider != null)
                slider.setAdjustValueMatcher(
                        this.midiIn.createAbsoluteCCValueMatcher(channel, this.sliderCcNumbers[i]));
        }
    }
    ...
    private void bindSlidersDrum(final int channel) {
        for (int i = 0; i < this.sliders.length; i++) {
            ...
            if (slider != null)
                slider.setAdjustValueMatcher(
                        this.midiIn.createAbsoluteCCValueMatcher(
                                channel, DrumMapping.SLIDER_CCS[i]));
        }
    }

    private void bindTransportAndTrack(
            final int channel,
            final int sendUpCc,
            final int sendDownCc,
            final int trackLeftCc,
    ...
    private void bindCC(final HardwareButton button, final Integer cc, final Integer channel) {
        ...
        button.pressedAction()
                .setActionMatcher(
                        this.midiIn.createCCActionMatcher(channel.intValue(), cc.intValue(), 127));
        button.releasedAction()
                .setActionMatcher(
                        this.midiIn.createCCActionMatcher(channel.intValue(), cc.intValue(), 0));
    }
    ...
    private void bindNote(final HardwareButton button, final Integer note, final Integer channel) {
        ...
        button.pressedAction()
                .setActionMatcher(
                        this.midiIn.createNoteOnActionMatcher(channel.intValue(), note.intValue()));
        button.releasedAction()
                .setActionMatcher(
                        this.midiIn.createNoteOffActionMatcher(
                                channel.intValue(), note.intValue()));
    }
}
