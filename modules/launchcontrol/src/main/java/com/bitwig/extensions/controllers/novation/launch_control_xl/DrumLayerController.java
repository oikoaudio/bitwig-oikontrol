 ...
 *
 * <p>Handles binding of hardware controls to drum pads (sliders -> pad volumes, knobs -> per-pad
 * remote controls, buttons -> triggers/mutes/solo) and provides a mute/solo mode that mirrors the
 * hardware Track Control buttons.
 */
final class DrumLayerController {
    ...
    /**
     * Bind all hardware controls to pads/parameters. Call once during initialization before the
        ...
        forEachPad(
                (index, pad) -> {
                    ...
                    final CursorRemoteControlsPage rc =
                            primaryDevice.createCursorRemoteControlsPage(4);
    ...
    /**
     * Activate the layer when the user template is selected. Verifies that the cursor device
     * exposes drum pads and shows guidance if not.
     */
    void engage() {
    ...
    void toggleSoloMode() {
        ...
        host.showPopupNotification(
                soloMode ? "Drum layer: Solo buttons" : "Drum layer: Default buttons");
    ...
    private void playPad(final DrumPad pad, final int padIndex, final int velocity) {
        ...
        // Derive the outgoing note purely from the current pad bank position and the pad index so
        // it
    ...
    void toggleMuteMode() {
        ...
        if (enableMute) this.soloMode = false;
        this.host.showPopupNotification(
                enableMute ? "Drum layer: Mute buttons" : "Drum layer: Default buttons");
    ...
    boolean handleMidi(final int status, final int data1, final int data2) {
        ...
        } else if (message == 0x90 || message == 0x80) // note on/off
        {
            ...
            if (index >= 0) {
                if (data2 > 0) handleBottomButton(index);
                else releaseBottomButton(index);
                return true;
            }
        }
        return false;
    ...
    /**
     * Handle a bottom-row button press. When the global track control mode is {@link
     * TrackControl#Mute} the button toggles the pad mute/solo. Otherwise it toggles the pad's
     * fourth remote parameter (useful for accent/roll assignments).
     */
    private void handleBottomButton(final int padIndex) {
        if (soloMode || trackControlMode == TrackControl.Mute) {
            ...
            if (pad.exists().get()) toggleMuteOrSolo(pad);
        } else {
            if (accentMomentary) setPadAccent(padIndex, true);
            else togglePadAccent(padIndex);
        }
    }

    private void releaseBottomButton(final int padIndex) {
        if (soloMode || trackControlMode == TrackControl.Mute || !accentMomentary) return;
    ...
    private void triggerMacro(final int macroIndex) {
        // Placeholder for future macro routing; currently consumes the event without external
        // action.
