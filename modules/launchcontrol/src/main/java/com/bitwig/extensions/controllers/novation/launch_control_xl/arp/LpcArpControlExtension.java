...
public class LpcArpControlExtension extends ControllerExtension implements ArpParameterContainer {
    ...
    private final byte[] buttonStateSysEx = {
        (byte) 0xF0, 0x00, 0x20, 0x29, 0x02, 0x11, 0x78, 0x00, 0x00, 0x00, (byte) 0xF7
    };
    ...
    protected LpcArpControlExtension(
            final LpcArpControlExtensionDefinition definition, final ControllerHost host) {
    ...
    public void init() {
                ...
                .addValueObserver(
                        s -> {
                            RemoteConsole.out.println(
                                    "Device Selected = {} {}", s, cursorDevice.presetName().get());
                            if (s.equals("Arpeggiator")) {
                                arpSelected = true;
                                currentMode.activate();
                                selectArpInstance(
                                        currentTrackIndex, cursorDevice.presetName().get());
        ...
        for (int i = 0; i < 8; i++) {
            focusButtons[i] =
                    new RedGreenButton(this, "FOCUS", i, TRACK_FOCUS_NOTE[i], BASE_CHANNEL);
            controlButtons[i] =
                    new RedGreenButton(this, "CONTROL", i, TRACK_CONTROL_NOTE[i], BASE_CHANNEL);
        }
        ...
        quantizeMode =
                documentState.getEnumSetting(
                        ...
                        new String[] {
                            ...
                            QuantizeMode.NEAREST_VALUE.getDescriptor()
                        },
                        QuantizeMode.MUTE.getDescriptor());
        quantizeMode.markInterested();
        quantizeMode.addValueObserver(
                newValue -> {
    ...
    private void setUpLayers() {
        ...
        final PatternTypeButtonMode patternTypeMode =
                new PatternTypeButtonMode(this, "PATTERNTYPE_LAYER");
        final VelocityGateButtonMode velGateMode =
                new VelocityGateButtonMode(this, "MUTE_GATE_MODE");

        final QuantizeButtonMode quantizeSettingMode =
                new QuantizeButtonMode(this, "QUANTIZE_MODE");
        final GlobalVelocityButtonMode velocityButtonMode =
                new GlobalVelocityButtonMode(this, "GLOBAL_VELOCITYMODE");
        final GlobalGateButtonMode gateButtonMode =
                new GlobalGateButtonMode(this, "GLOBAL_GATEMODE");
    ...
    public void updatePadLed(final RedGreenButton button) {
        final ColorButtonLedState state =
                (ColorButtonLedState) button.getLight().state().currentValue();
        if (state != null) {
    ...
    private void setUpKnobs() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            knob.setAdjustValueMatcher(
                    midiIn.createAbsoluteCCValueMatcher(BASE_CHANNEL, SENDA_CC_INDEX + i));
            knob.value()
                    .addValueObserver(
                            v -> {
                                if (currentArp != null) {
                                    ...
                                    arpNoteParams
                                            .get(index)
                                            .value()
                                            .set(currentArp.getNoteValue(index));
                                }
                            });
        }
        for (int i = 0; i < 8; i++) {
            ...
            sendBKnobs[i] = knob;
            knob.setAdjustValueMatcher(
                    midiIn.createAbsoluteCCValueMatcher(BASE_CHANNEL, SENDB_CC_INDEX + i));
            knob.value()
                    .addValueObserver(
                            value -> {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            panKnobs[i] = knob;
            knob.setAdjustValueMatcher(
                    midiIn.createAbsoluteCCValueMatcher(BASE_CHANNEL, PAN_CC_INDEX + i));
            knob.value()
                    .addValueObserver(
                            value -> {
                                if (currentArp != null) {
                                    currentArp.updateGateParam(
                                            index, arpGateParams.get(index), value);
                                }
                            });
        }
        ...
        for (int i = 0; i < 8; i++) {
            ...
            sliders[i] = knob;
            knob.setAdjustValueMatcher(
                    midiIn.createAbsoluteCCValueMatcher(BASE_CHANNEL, SLIDER_CC_INDEX + i));
            knob.value()
                    .addValueObserver(
                            v -> {
                                if (currentArp != null) {
    ...
    private void setUpArpDevice(final PinnableCursorDevice cursorDevice) {
        arpdevice =
                cursorDevice.createSpecificBitwigDevice(
        ...
        //		final Parameter terminateNotes =
        // arpdevice.createParameter("IMMEDIATELY_TERMINATE_NOTES");
        ...
        for (int i = 0; i < 8; i++) {
            ...
            final MultiStateHardwareLight light =
                    surface.createMultiStateHardwareLight("SENDA_LIGHT_" + i);
            ...
            light.state()
                    .onUpdateHardware(
                            hwState -> {
                                if (currentArp != null) {
                                    final ColorButtonLedState state =
                                            (ColorButtonLedState) light.state().currentValue();
                                    sendLightState(0, index, state.getColorCode());
                                } else {
                                    sendLightState(
                                            0, index, ColorButtonLedState.OFF.getColorCode());
                                }
                            });
            noteParm.value()
                    .addRawValueObserver(
        ...
        for (int i = 0; i < 8; i++) {
            ...
            final MultiStateHardwareLight light =
                    surface.createMultiStateHardwareLight("SENDB_LIGHT_" + i);
        ...
        for (int i = 0; i < 8; i++) {
            final int index = i;
            final MultiStateHardwareLight light =
                    surface.createMultiStateHardwareLight("PAN_LIGHT_" + i);
