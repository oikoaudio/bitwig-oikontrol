...
final class DevicePagesController {
    ...
    DevicePagesController(
            final CursorRemoteControlsPage[] devicePages,
    ...
    void paintButtons() {
        ...
        for (int i = 0; i < 8; ++i) {
            setLedForRemote(
                    bottomButtonLeds[i], devicePages[4].getParameter(i), off, amberLow, amber);
            setLedForRemote(
                    bottomButtonLeds[8 + i], devicePages[5].getParameter(i), off, amberLow, amber);
        }
    }

    private void handleCc(final int cc, final int value) {
        final Integer row0Index =
                knobIndex(cc, LaunchControlXlControllerExtension.KNOB_CC_OFFSETS[0]);
        if (row0Index != null) {
            LaunchControlXlControllerExtension.logDebug(
                    "[LCXL-DP] row0 idx=" + row0Index + " cc=" + cc + " val=" + value);
            setRemoteValue(devicePages[0].getParameter(row0Index), value);
            return;
        }
        final Integer row1Index =
                knobIndex(cc, LaunchControlXlControllerExtension.KNOB_CC_OFFSETS[1]);
        if (row1Index != null) {
            LaunchControlXlControllerExtension.logDebug(
                    "[LCXL-DP] row1 idx=" + row1Index + " cc=" + cc + " val=" + value);
            setRemoteValue(devicePages[1].getParameter(row1Index), value);
            return;
        }
        final Integer row2Index =
                knobIndex(cc, LaunchControlXlControllerExtension.KNOB_CC_OFFSETS[2]);
        if (row2Index != null) {
            LaunchControlXlControllerExtension.logDebug(
                    "[LCXL-DP] row2 idx=" + row2Index + " cc=" + cc + " val=" + value);
        ...
        if (slider != null) {
            LaunchControlXlControllerExtension.logDebug(
                    "[LCXL-DP] fader idx=" + slider + " cc=" + cc + " val=" + value);
    ...
    private void handleNote(final int note) {
        ...
        final int controlIndex =
                indexOf(LaunchControlXlControllerExtension.TRACK_CONTROL_NOTES, note);
        if (controlIndex >= 0) {
