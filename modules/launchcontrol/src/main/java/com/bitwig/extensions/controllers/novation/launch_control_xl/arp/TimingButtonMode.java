...
public class TimingButtonMode extends ButtonModeLayer {
    ...
    public TimingButtonMode(final LpcArpControlExtension driver, final String name) {
        ...
        bindLightState(
                () ->
                        rateMode.getRaw() == 0
                                ? ColorButtonLedState.AMBER_FULL
                                : ColorButtonLedState.AMBER_DIM,
                focusButtons[0]);
        bindLightState(
                () ->
                        rateMode.getRaw() == 1
        ...
        bindLightState(
                () ->
                        shuffle.getRaw() == 0
                                ? ColorButtonLedState.ORANGE_FULL
                                : ColorButtonLedState.AMBER_DIM,
                focusButtons[4]);
        bindLightState(
                () -> {
                    return cursorDevice.isPinned().get()
                            ? ColorButtonLedState.RED_FULL
                            : ColorButtonLedState.RED_DIM;
                },
                focusButtons[7]);
        ...
        for (int i = 0; i < 7; i++) {
            ...
            bindLightState(
                    () ->
                            arpRate.getRaw() == index
                                    ? ColorButtonLedState.GREEN_FULL
                                    : ColorButtonLedState.GREEN_DIM,
                    controlButtons[i]);
        }
        bindLightState(
                () ->
