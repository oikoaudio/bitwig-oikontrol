...
public record FactoryUiSnapshot(
        Surface surface,
        Mode mode,
    ...
    @Override
    public Value[] deviceRemotes() {
        return deviceRemotes.clone();
    }

    @Override
    public Value[] projectRemotes() {
        return projectRemotes.clone();
    }
    ...
    public FactoryUiSnapshot withDeviceOn(final boolean value) {
        return new FactoryUiSnapshot(
                ...
                projectRemotes,
                sendCanScrollBack,
                sendCanScrollForward,
                trackCanScrollBack,
                trackCanScrollForward,
                deviceHasPrevious,
                deviceHasNext);
    }

    public FactoryUiSnapshot withNavigation(
            final boolean sendBack,
            final boolean sendForward,
            final boolean trackBack,
            final boolean trackForward,
            final boolean devicePrevious,
            final boolean deviceNext) {
        return new FactoryUiSnapshot(
                surface,
                mode,
                trackControl,
    ...
    public record Strip(
        ...
        @Override
        public Value[] deviceParameters() {
            return deviceParameters.clone();
        }

        @Override
        public Value[] trackParameters() {
            return trackParameters.clone();
        }

        public static Strip existing(
                final boolean mute,
                final boolean solo,
                final boolean arm,
                final Value trackControl,
                final Value[] sends,
                final Value[] deviceParameters,
                final Value[] trackParameters) {
            return new Strip(
                    true, mute, solo, arm, trackControl, sends, deviceParameters, trackParameters);
        }

        public static Strip missing() {
            final Value[] missing = {Value.missing(), Value.missing(), Value.missing()};
            return new Strip(
                    false, false, false, false, Value.missing(), missing, missing, missing);
        }
    }
}
