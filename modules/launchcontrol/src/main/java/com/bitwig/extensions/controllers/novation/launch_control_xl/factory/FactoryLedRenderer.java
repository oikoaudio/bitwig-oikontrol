...
public final class FactoryLedRenderer {
    ...
    public record LedFrame(
        ...
        @Override
        public int[] topButtons() {
            return topButtons.clone();
        }

        @Override
        public int[] bottomButtons() {
            return bottomButtons.clone();
        }
    ...
    private static int bottomColor(
            final FactoryUiSnapshot state, final FactoryUiSnapshot.Strip strip, final int index) {
        if (state.deviceOn()) {
            if (state.selectedRemotePage() == index) return color(SimpleLedColor.Amber);
            return index < state.remotePageCount()
                    ? color(SimpleLedColor.AmberLow)
                    : color(SimpleLedColor.Off);
        }
        if (!strip.exists()) return color(SimpleLedColor.Off);
        return switch (state.trackControl()) {
            case MUTE ->
                    strip.mute() ? color(SimpleLedColor.GreenLow) : color(SimpleLedColor.Green);
            case SOLO ->
                    strip.solo() ? color(SimpleLedColor.Amber) : color(SimpleLedColor.AmberLow);
            case RECORD_ARM ->
                    strip.arm() ? color(SimpleLedColor.Red) : color(SimpleLedColor.RedLow);
            case NONE -> level(strip.trackControl(), SimpleLedColor.AmberLow, SimpleLedColor.Amber);
        };
    }

    private static void renderKnobs(
            final FactoryUiSnapshot state,
            final FactoryUiSnapshot.Strip strip,
            final int index,
            final int[] knobs,
            final FactoryUiSnapshot.Value[] deviceRemotes,
            final FactoryUiSnapshot.Value[] projectRemotes) {
        if (!strip.exists() || state.mode() == FactoryUiSnapshot.Mode.NONE) return;
        ...
        switch (state.mode()) {
            case SEND_2_DEVICE_1 ->
                    set(knobs, index, send(sends, 0), send(sends, 1), device(device, 0));
            case SEND_2_PAN_1 ->
                    set(knobs, index, send(sends, 0), send(sends, 1), color(SimpleLedColor.Red));
            case SEND_3 -> set(knobs, index, send(sends, 0), send(sends, 1), send(sends, 2));
            case SEND_1_DEVICE_2 ->
                    set(knobs, index, send(sends, 0), device(device, 0), device(device, 1));
            case DEVICE_3 ->
                    set(knobs, index, device(device, 0), device(device, 1), device(device, 2));
            case TRACK_3 -> set(knobs, index, device(track, 0), device(track, 1), device(track, 2));
            case SEND_2_FULL_DEVICE ->
                    set(knobs, index, send(sends, 0), send(sends, 1), device(deviceRemotes, index));
            case SEND_2_PROJECT ->
                    set(
                            knobs,
                            index,
    ...
    private static int[] modeAndNavigationButtons(final FactoryUiSnapshot state) {
        ...
        return new int[] {
            ...
            state.sendCanScrollBack() ? on : off,
            state.sendCanScrollForward() ? on : off,
            state.deviceOn()
                    ? (state.deviceHasPrevious() ? on : off)
                    : (state.trackCanScrollBack() ? on : off),
            state.deviceOn()
                    ? (state.deviceHasNext() ? on : off)
                    : (state.trackCanScrollForward() ? on : off)
        };
    }
    ...
    private static FactoryUiSnapshot.Value value(
            final FactoryUiSnapshot.Value[] values, final int index) {
    ...
    private static int level(
            final FactoryUiSnapshot.Value value,
            final SimpleLedColor dim,
            final SimpleLedColor bright) {
        return value != null && value.exists()
                ? LedUtil.levelColor(
                        value.value(), color(SimpleLedColor.Off), color(dim), color(bright))
                : color(SimpleLedColor.Off);
    }

    private static void set(
            final int[] knobs, final int index, final int top, final int middle, final int bottom) {
        knobs[index] = top;
        knobs[8 + index] = middle;
        knobs[16 + index] = bottom;
    }

    private static int color(final SimpleLedColor color) {
        return color.value();
    }
}
