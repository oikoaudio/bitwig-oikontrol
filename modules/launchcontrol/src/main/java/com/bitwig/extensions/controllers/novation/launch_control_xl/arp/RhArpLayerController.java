...
public final class RhArpLayerController {
    ...
    private static final double[] GLOBAL_VELOCITY_VALUES = {
        0, 0.1, 0.25, 0.40, 0.55, 0.75, 0.9, 1.0
    };
    ...
    private int globalVelocityColor(final int index) {
        ...
        final double target =
                index < GLOBAL_VELOCITY_VALUES.length ? GLOBAL_VELOCITY_VALUES[index] : -1;
    ...
    private int timingFocusColor(final int index) {
        if (index >= 0 && index <= 2) {
            ...
            return Math.abs(raw - index) < 0.001
                    ? SimpleLedColor.Amber.value()
                    : SimpleLedColor.AmberLow.value();
        }
        if (index == 4) {
            return shuffleParameter.value().getRaw() == 0
                    ? SimpleLedColor.Red.value()
                    : SimpleLedColor.RedLow.value();
        }
    ...
    private int timingControlColor(final int index) {
        ...
        if (index == 7) {
            return retriggerParameter.value().getRaw() == 1
                    ? SimpleLedColor.Amber.value()
                    : SimpleLedColor.AmberLow.value();
        }
        return SimpleLedColor.Off.value();
    ...
    private int patternColor(final int index) {
        ...
        return Math.abs(raw - (index + 1)) < 0.001
                ? SimpleLedColor.Red.value()
                : SimpleLedColor.RedLow.value();
    }
    ...
    private int velocityToggleColor(final int index) {
        ...
        return velocityBoostEnabled[index]
                ? SimpleLedColor.Green.value()
                : SimpleLedColor.GreenLow.value();
    }
