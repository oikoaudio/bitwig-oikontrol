 ...
 *
 * <p>This class wires all hardware controls to Bitwig features by creating layers (factory modes,
 * drum layer, arp layer, etc.), reacts to template changes, and coordinates helpers such as {@link
 * DeviceLocator} and {@link DrumLayerController}. Keep most view-specific logic in those helpers so
 * this class remains the orchestrator.
 */
public class LaunchControlXlControllerExtension extends ControllerExtension {
    ...
    enum Mode {
        ...
        None(
                0,
                "Unsupported Template. We provide Modes for the Factory Template 1 to 8, except 7."),
    ...
    public void init() {
        ...
        mExclusiveTrackArm.addValueObserver(
                value -> {
        ...
        mCursorTrack =
                mHost.createCursorTrack(
                        "cursor-track", "Launch Control XL Track Cursor", 0, 0, true);
        ...
        mProjectRemoteControlsCursor =
                rootTrackGroup.createCursorRemoteControlsPage("project-remotes", 8, null);
        ...
        mDrumSettings
                ...
                .addValueObserver(this::applyTrackVisibilityPreference);
        ...
        for (int i = 0; i < STRIP_COUNT; ++i) {
            ...
            if (mDevicePerTrackSupported) {
                ...
                mTrackCursorDeviceRemoteControls[i] =
                        mTrackDeviceCursors[i].createCursorRemoteControlsPage(4);
    ...
    private void initDeviceRemotePages() {
        for (int pageIndex = 0; pageIndex < mDeviceRemotePages.length; pageIndex++) {
            final CursorRemoteControlsPage rc =
                    mCursorDevice.createCursorRemoteControlsPage(
                            "device-page-" + (pageIndex + 1), 8, "");
    ...
    private void createHardwareSurface() {
        ...
        for (int i = 0; i < 8; ++i) {
            for (int j = 0; j < 3; ++j) {
                final AbsoluteHardwareKnob knob =
                        mHardwareSurface.createAbsoluteHardwareKnob("knob-" + i + "-" + j);
        ...
        for (int i = 0; i < mBtTrackFocus.length; i++) {
            mBtTrackFocus[i] =
                    createHardwareButtonWithNote("bt-track-focus-" + i, TRACK_FOCUS_NOTES[i], i);
            mBtTrackFocus[i].isPressed().markInterested();
        }

        for (int i = 0; i < mBtTrackControl.length; i++) {
            mBtTrackControl[i] =
                    createHardwareButtonWithNote(
                            "bt-track-control-" + i, TRACK_CONTROL_NOTES[i], i);
    ...
    private HardwareButton createHardwareButtonWithNote(
            final String id, final int note, final int indexInGroup) {
    ...
    private void setNoteButtonActionMatchers(
            final HardwareButton button, final int note, final Integer channel) {
        if (channel == null) {
    ...
    private void setCcButtonActionMatchers(
            final HardwareButton button, final int cc, final Integer channel) {
        if (channel == null) {
            button.pressedAction().setActionMatcher(null);
            button.releasedAction().setActionMatcher(null);
            return;
    ...
    private void setCcButtonMatcher(
            final HardwareButton button, final int cc, final Integer channel) {
        if (channel == null) {
            button.pressedAction().setActionMatcher(null);
            return;
    ...
    private void attachHardwareMatchers() {
        ...
        final int channel =
                mDrumLayerActive
                        ? DRUM_USER_TEMPLATE_ID
                        : mDevicePagesLayerActive
                                ? DEVICE_PAGES_USER_TEMPLATE_ID
                                : mCurrentTemplateChannel;
        mHostActions.debug(
                "[LCXL] attachHardwareMatchers mode="
                        + mMode
                        + " channel="
                        + channel
                        + " factoryActive="
                        + mFactoryTemplateActive
                        + " arpActive="
                        + mArpLayerActive
                        + " drumActive="
                        + mDrumLayerActive
    ...
    static boolean hardwareAttachShouldDefer(
            final boolean hardwareReady, final boolean bindingManagerReady) {
    ...
    private void createLayers() {
        ...
        for (int i = 0; i < STRIP_COUNT; ++i) {
            ...
            mMainLayer.bind(
                    mHardwareSliders[i],
                    value ->
                            mFactoryLayerController.setTrackValue(
                                    stripIndex,
                                    FactoryLayerController.TrackValueTarget.VOLUME,
                                    value));
            mMainLayer.bindPressed(
                    mBtTrackFocus[i], () -> mFactoryLayerController.selectTrack(stripIndex));
        }
        ...
        mMainLayer.bindPressed(
                mBtMute,
                () ->
                        mFactoryLayerController.toggleTrackControl(
        ...
        mMainLayer.bindPressed(
                ...
                () ->
                        mFactoryLayerController.toggleTrackControl(
                                FactoryUiSnapshot.TrackControl.RECORD_ARM));
    ...
    private void createDeviceLayer(final Layers layers) {
        ...
        mDeviceLayer.bindPressed(
                mBtTrackLeft, () -> mFactoryLayerController.selectPreviousDevice());
        mDeviceLayer.bindPressed(mBtTrackRight, () -> mFactoryLayerController.selectNextDevice());

        for (int i = 0; i < 8; ++i) {
            final int I = i;
            mDeviceLayer.bindPressed(
                    mBtTrackControl[i], () -> mFactoryLayerController.selectDeviceRemotePage(I));
        }
    }

    /**
     * Create the drum layer (user template 7) and wire all hardware controls to the {@link
     * DrumLayerController}. This layer takes over knobs/sliders/buttons when the drum template is
     * active, so the helper receives references to all relevant controls.
     */
    private void createDrumLayer(final Layers layers) {
        mDrumLayer = new Layer(layers, "Drum");

        final AbsoluteHardwareKnob[][] drumKnobs =
                new AbsoluteHardwareKnob[3][DrumLayerController.PADS_PER_BANK];
        ...
        mDrumLayerController =
                new DrumLayerController(
                        mHost,
                        mUserModeNoteInput,
                        DRUM_USER_TEMPLATE_ID,
                        mDrumPadBank,
    ...
    private void createDevicePagesLayer(final Layers layers) {
        ...
        mDevicePagesController =
                new DevicePagesController(
                        mDeviceRemotePages,
                        mHardwareKnobs,
                        mHardwareSliders,
                        mKnobsLed,
        ...
        mDevicePagesLayer.bindPressed(
                mBtSendUp, () -> bumpRemoteParameter(mDeviceRemotePages[6].getParameter(0), 1));
        mDevicePagesLayer.bindPressed(
                mBtSendDown, () -> bumpRemoteParameter(mDeviceRemotePages[6].getParameter(0), -1));
        mDevicePagesLayer.bindPressed(
                mBtTrackLeft, () -> bumpRemoteParameter(mDeviceRemotePages[6].getParameter(1), -1));
        mDevicePagesLayer.bindPressed(
                mBtTrackRight, () -> bumpRemoteParameter(mDeviceRemotePages[6].getParameter(1), 1));
    ...
    private void createModeLayers(final Layers layers) {
        ...
        for (int i = 0; i < STRIP_COUNT; ++i) {
            final int stripIndex = i;
            mSend2FullDeviceLayer.bind(
                    mHardwareKnobs[i],
                    value -> mFactoryLayerController.setSendValue(stripIndex, 0, value));
            mSend2FullDeviceLayer.bind(
                    mHardwareKnobs[8 + i],
                    value -> mFactoryLayerController.setSendValue(stripIndex, 1, value));
            mSend2FullDeviceLayer.bind(
                    mHardwareKnobs[16 + i],
                    value ->
                            mFactoryLayerController.setSelectedDeviceRemoteValue(
                                    stripIndex, value));
        }
        ...
        for (int i = 0; i < STRIP_COUNT; ++i) {
            ...
            mDevice3Layer.bind(
                    mHardwareKnobs[16 + i],
                    value -> mFactoryLayerController.setDeviceRemoteValue(stripIndex, 2, value));
        }

        mSend2Pan1Layer = new Layer(layers, "2 Sends 1 Pan");
        for (int i = 0; i < STRIP_COUNT; ++i) {
            final int stripIndex = i;
            mSend2Pan1Layer.bind(
                    mHardwareKnobs[i],
                    value -> mFactoryLayerController.setSendValue(stripIndex, 0, value));
            mSend2Pan1Layer.bind(
                    mHardwareKnobs[8 + i],
            ...
            mSend2Pan1Layer.bind(
                    ...
                    value ->
                            mFactoryLayerController.setTrackValue(
                                    stripIndex,
                                    FactoryLayerController.TrackValueTarget.PAN,
                                    value));
        }
        ...
        for (int i = 0; i < STRIP_COUNT; ++i) {
            ...
            mSend3Layer.bind(
                    mHardwareKnobs[16 + i],
                    value -> mFactoryLayerController.setSendValue(stripIndex, 2, value));
        }
        ...
        for (int i = 0; i < STRIP_COUNT; ++i) {
            ...
            mTrack3layer.bind(
                    mHardwareKnobs[i],
                    value -> mFactoryLayerController.setTrackRemoteValue(stripIndex, 0, value));
            mTrack3layer.bind(
                    mHardwareKnobs[8 + i],
                    value -> mFactoryLayerController.setTrackRemoteValue(stripIndex, 1, value));
            mTrack3layer.bind(
                    mHardwareKnobs[16 + i],
                    value -> mFactoryLayerController.setTrackRemoteValue(stripIndex, 2, value));
        }
    }
    ...
    private void createTrackControlsLayers(final Layers layers) {
        ...
        for (int i = 0; i < STRIP_COUNT; ++i) {
            final int stripIndex = i;
            mMuteLayer.bindPressed(
                    mBtTrackControl[i],
                    () ->
                            mFactoryLayerController.toggleTrackBoolean(
        ...
        for (int i = 0; i < 8; ++i) {
            final int stripIndex = i;
            mTrackRemoteButtonLayer.bindPressed(
                    mBtTrackControl[i],
    ...
    private void handleIncomingMidi(final int status, final int data1, final int data2) {
        ...
        if (mDrumLayerActive
                && mDrumLayerController != null
                && mDrumLayerController.handleMidi(status, data1, data2)) {
            return;
        }
        if (mDrumLayerActive) {
            mHostActions.debug(
                    String.format(
                            "[LCXL] DRUM MIDI status=%02X data1=%02X data2=%02X (channel %d)",
                            status, data1, data2, status & 0x0F));
        }
        if (mDevicePagesLayerActive) {
            mHostActions.debug(
                    String.format(
                            "[LCXL] DEVICE-PAGES MIDI status=%02X data1=%02X data2=%02X (channel %d)",
                            status, data1, data2, status & 0x0F));
        }
        if (mFactoryTemplateActive) {
            mHostActions.debug(
                    String.format(
                            "[LCXL] MIDI status=%02X data1=%02X data2=%02X", status, data1, data2));
        }
    }
    ...
    private void setTrackValue(
            final int stripIndex, final TrackValueTarget target, final double value) {
    ...
    static boolean trackBooleanShouldSelect(
            final TrackBooleanTarget target, final boolean exclusiveTrackArmEnabled) {
    ...
    private RemoteControl trackParamForStrip(final int stripIndex, final int paramIndex) {
        return trackForStrip(stripIndex) != null
                ? mTrackRemoteControls[stripIndex].getParameter(paramIndex)
                : null;
    }
    ...
    private boolean isControllableTrack(final Track track) {
        return track != null
                && track.exists().get()
                && (mDrumSettings != null && mDrumSettings.showDeactivatedTracksEnabled()
    ...
    private void applyTrackVisibilityPreference(final boolean showDeactivatedTracks) {
        if (mTrackBank != null) {
            mTrackBank.setSkipDisabledItems(
                    trackVisibilityShouldSkipDisabled(showDeactivatedTracks));
        }
    }

    /**
     * Returns a device remote parameter for the given track slot, falling back to track remotes
     * when per-track cursor devices are unavailable (Bitwig 6 compatibility).
     */
     ...
     * requested) before scanning via the {@link DeviceLocator}, then applies the focus (select
     * track, select device, notify cursor device).
     *
    ...
    private boolean focusDevice(final Role role, final boolean tryCacheFirst) {
        if (this.mDeviceLocator == null) return false;

        Optional<FocusResult> focusResult = Optional.empty();
        if (tryCacheFirst) focusResult = this.mDeviceLocator.focusCached(role);

        if (focusResult.isEmpty()) focusResult = this.mDeviceLocator.focusFirst(role);
    ...
    private void applyFocusedDevice(final Role role, final FocusResult focus) {
        ...
        if (this.mCursorDevice != null) this.mCursorDevice.selectDevice(focus.device());

        this.mHostActions.debug(
                "[LCXL] Focused " + getRoleLabel(role) + " on track index " + focus.trackIndex());
    }
    ...
    private void setArpLayerActive(final boolean active) {
        ...
        if (active) {
            ...
            if (mCursorDevice != null) mCursorDevice.isPinned().set(attached);
    ...
    /**
     * Engage or disengage the drum user layer (user template 7). When enabling, attempt to
     * auto-focus the drum machine track/device and rebind hardware controls so the {@link
     * DrumLayerController} owns the knobs/sliders/buttons.
     */
    private void setDrumLayerActive(final boolean active) {
        ...
        if (active) {
            mHostActions.debug(
                    "[LCXL] drum layer engage request (user template 7) currentTemplate="
                            + mCurrentTemplateChannel);
            if (mAutoAttachToFirst == null || mAutoAttachToFirst.get()) {
    ...
    /**
     * Engage or disengage the single-device pages user layer (user template 6). Rebinds the
     * hardware to the dedicated remote pages and shows a popup when activated.
     */
    ...
    private void selectModeFromSysex(final String sysex) {
        ...
        if (!sysex.startsWith(prefix)) return;
        ...
        if (8 <= ch && ch <= 15) {
            for (final Mode mode : Mode.values()) {
                if (mode.getChannel() == ch) {
                    mHostActions.debug(
                            "[LCXL] factory template channel " + ch + " -> mode " + mode.name());
                    selectMode(mode);
                    if (mPendingAttach) {
                        attachHardwareMatchers();
                    }
            ...
            mHostActions.debug(
                    "[LCXL] No factory mode mapped for channel "
                            + ch
                            + " (factory template not supported)");
            mFactoryTemplateActive = false;
            setFactoryLayersEnabled(false);
            setArpLayerActive(false);
            selectMode(Mode.None);
        } else {
            // User templates (0–7) report other channel codes. We simply ignore the mode change so
            // the
            // template-change handler can deal with enabling/disabling the layers, but we still log
            // it
    ...
    private final class FactoryPort implements FactoryLayerController.Port {
        ...
        public void setTrackArm(final int strip, final boolean value) {
            final Track track = trackForStrip(strip);
            if (track != null) track.arm().set(value);
        }

        @Override
        public void toggleTrackMute(final int strip) {
            final Track track = trackForStrip(strip);
            if (track != null) track.mute().toggle();
        }

        @Override
        public void toggleTrackSolo(final int strip) {
            final Track track = trackForStrip(strip);
            if (track != null) track.solo().toggle(false);
        }

        @Override
        public void selectTrack(final int strip) {
            final Track track = trackForStrip(strip);
            if (track != null) mCursorTrack.selectChannel(track);
        }

        @Override
        public void scrollSendBank(final int strip, final int direction) {
            final SendBank sendBank = sendBankForStrip(strip);
            if (sendBank == null) return;
            if (direction < 0) sendBank.scrollBackwards();
            else if (direction > 0) sendBank.scrollForwards();
        }

        @Override
        public boolean canScrollTrackPage(final int direction) {
            return direction < 0
                    ? mTrackBank.canScrollBackwards().get()
                    : direction > 0 && mTrackBank.canScrollForwards().get();
        }

        @Override
        public void scrollTrackPage(final int direction) {
            if (direction < 0) mTrackBank.scrollPageBackwards();
            else if (direction > 0) mTrackBank.scrollPageForwards();
        }

        @Override
        public void scheduleFirstTrackSelection() {
            mHost.scheduleTask(
                    () -> mFactoryLayerController.selectTrack(0), TRACK_PAGE_SELECTION_DELAY_MS);
        }

        @Override
        public void setTrackValue(
                final int strip,
                final FactoryLayerController.TrackValueTarget target,
                final double value) {
            LaunchControlXlControllerExtension.this.setTrackValue(
                    strip,
                    target == FactoryLayerController.TrackValueTarget.VOLUME
                            ? TrackValueTarget.VOLUME
                            : TrackValueTarget.PAN,
                    value);
        }
        ...
        @Override
        public void setSendBankSize(final int size) {
            setSizeOfSendBank(size);
        }
    ...
    private void onSysex(final String sysex) {
        ...
        // Pass sysex to the arp layer first so it can react to its own messages (e.g., timing
        // changes).
        ...
        if (templateId.isPresent()) {
            ...
            mHostActions.debug(
                    "[LCXL] onSysex templateId="
                            + id
                            + " arpTemplate="
                            + (id == ARP_USER_TEMPLATE_ID)
                            + " factory="
                            + (id >= 8));
            if (id < 8) {
                ...
                if (arpTemplate) {
                    mHostActions.debug(
                            "[LCXL] handleTemplateChange -> arp template (user template 8)");
                } else {
                    mHostActions.debug(
                            "[LCXL] handleTemplateChange -> user template "
                                    + (id + 1)
                                    + (drumTemplate
                                            ? " (drum)"
                                            : devicePagesTemplate ? " (device pages)" : ""));
                }
                mFactoryTemplateActive = false;
                setFactoryLayersEnabled(false);
    ...
    public void flush() {
        ...
        for (final SimpleLed simpleLed : mKnobsLed) simpleLed.flush(sb);

        for (final SimpleLed simpleLed : mBottomButtonsLed) simpleLed.flush(sb);

        if (!sb.toString().isEmpty()) {
            final int channel =
                    mFactoryTemplateActive ? mMode.getChannel() : mCurrentTemplateChannel;
            final String sysex =
                    "F0 00 20 29 02 11 78 0" + Integer.toHexString(channel) + sb + " F7";
    ...
    private FactoryUiSnapshot factoryUiSnapshot() {
        ...
        for (int i = 0; i < STRIP_COUNT; i++) {
            ...
            if (track == null) {
                ...
                for (int parameter = 0; parameter < 3; parameter++) {
                    sendValues[parameter] =
                            sends != null && sends.getItemAt(parameter).exists().get()
                                    ? FactoryUiSnapshot.Value.of(
                                            sends.getItemAt(parameter).value().get())
                ...
                strips[i] =
                        FactoryUiSnapshot.Strip.existing(
                                ...
                                track.solo().get(),
                                track.arm().get(),
                                snapshotValue(trackParamForStrip(i, 3)),
                                sendValues,
                                deviceValues,
                                trackValues);
            }
        ...
        return new FactoryUiSnapshot(
                factorySurface(),
                factoryMode(),
                mFactoryLayerController.trackControl(),
                mFactoryLayerController.deviceOn(),
                mRemoteControls.selectedPageIndex().get(),
                mRemoteControls.pageCount().get(),
                mTrackBank.cursorIndex().get(),
                strips,
                deviceRemotes,
                projectRemotes,
                ...
                mTrackBank.canScrollBackwards().get(),
                mTrackBank.canScrollForwards().get(),
                mCursorDevice.hasPrevious().get(),
                mCursorDevice.hasNext().get());
    }
    ...
    private FactoryUiSnapshot.Surface factorySurface() {
        return factorySurface(
                mFactoryTemplateActive, mArpLayerActive, mDrumLayerActive, mDevicePagesLayerActive);
    }

    static FactoryUiSnapshot.Surface factorySurface(
            final boolean factoryTemplateActive,
            final boolean arpLayerActive,
            final boolean drumLayerActive,
            final boolean devicePagesLayerActive) {
        if (drumLayerActive) return FactoryUiSnapshot.Surface.DRUM;
        if (arpLayerActive) return FactoryUiSnapshot.Surface.ARP;
        if (devicePagesLayerActive) return FactoryUiSnapshot.Surface.DEVICE_PAGES;
        return factoryTemplateActive
                ? FactoryUiSnapshot.Surface.FACTORY
                : FactoryUiSnapshot.Surface.RAW_USER;
    }

    private static TrackControl fromFactoryTrackControl(
            final FactoryUiSnapshot.TrackControl trackControl) {
    ...
    private void paintDrumButtons() {
        ...
        final TrackControl drumControlMode =
                mDrumLayerController != null
                        ? mDrumLayerController.getTrackControlMode()
                        : TrackControl.None;
        final int selectedPad =
                mDrumLayerController != null ? mDrumLayerController.getSelectedPadIndex() : -1;

        final DrumLedRenderer.PadState[] pads =
                new DrumLedRenderer.PadState[DrumLayerController.PADS_PER_BANK];
        ...
        final DrumUiState state =
                new DrumUiState(pads, selectedPad, soloMode, drumControlMode == TrackControl.Mute);
    ...
    private void paintDevicePagesKnobs() {
        if (mDevicePagesController != null) mDevicePagesController.paintKnobs();
    }

    private void paintDevicePagesButtons() {
        if (mDevicePagesController != null) mDevicePagesController.paintButtons();
    }
    ...
    private void paintKnobs(final FactoryLedRenderer.LedFrame factoryFrame) {
        ...
        for (int i = 0; i < colors.length; i++) mKnobsLed[i].setColor(colors[i]);
    }
    ...
    private void paintModeAndNavigationButtons(final FactoryLedRenderer.LedFrame factoryFrame) {
        ...
        if (mDrumLayerActive) {
            final boolean canScrollBack =
                    mDrumPadBank != null && mDrumPadBank.canScrollBackwards().get();
            final boolean canScrollForward =
                    mDrumPadBank != null && mDrumPadBank.canScrollForwards().get();
            ...
            mSoloLed.setColor(
                    mDrumLayerController != null && mDrumLayerController.isSoloMode()
                            ? yellow
                            : off);
    ...
    private final CursorRemoteControlsPage[] mTrackCursorDeviceRemoteControls =
            new CursorRemoteControlsPage[STRIP_COUNT];
    private final CursorRemoteControlsPage[] mTrackRemoteControls =
            new CursorRemoteControlsPage[STRIP_COUNT];
    ...
    private final SimpleLed[] mKnobsLed =
            new SimpleLed[] {
    ...
    private final SimpleLed[] mBottomButtonsLed =
            new SimpleLed[] {
