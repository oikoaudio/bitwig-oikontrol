...
| --- | --- | --- | --- | --- |
| `Channel` | Density / `ALT`: chance / `SHIFT`: recurrence | Tuplet / `ALT`: Tup.Div / `SHIFT`: target phase | Ratchet / `ALT`: Rat.Div / `SHIFT`: target phase | Cluster / `ALT`: play start / `SHIFT`: rate |
...
| `User 2` | Linear pitch | Pitch Expr spread or held-hit pitch expr / `ALT`: center / `SHIFT`: rotate | Clip length / `ALT`: play start / `SHIFT`: rate | Reset hit edits |
...
Tuplet is usually a half-bar transform: `Tuplet` sets how many target spans are affected, while `Tup.Div` sets how many divisions each selected span is split into. In compound eighth-note meters, each tuplet target span follows the dotted-quarter structural beat instead. Controller feedback shows target count versus split count, for example `4 spans` means four target spans and `x5` means each selected span is divided into five. Tuplet and ratchet target order are both derived from the whole clip as a single phrase, but they use complementary profiles: tuplets claim broad phrase spans, while ratchets first decorate structural and edge cells around those spans and only overlap into tuplet cells as the ratchet target count grows. `Target Phase` rotates through the relevant target order. Ratchet comes after tuplets: `Ratchet` sets how many parent regions are affected, while `Rat.Div` sets how many divisions each selected parent region is split into. Parent regions are the beat cells plus any tuplet cells created inside the selected spans, so `4 cells` means four parent regions are selected and `x4` means each selected region is split into four. Very small parent cells are skipped when the selected ratchet division would create sub-thirty-second-note rolls. Tuplet divisions are meter- and rate-aware, expanding up to `x12` where the count creates a non-native division of the current span; in `4/4` they include `3 / 5 / 7 / 9 / 11`, while `6/8` can expose divisions such as `4 / 5 / 7 / 8 / 10 / 11`. Ratchet divisions support `2 / 3 / 4 / 5 / 6 / 7 / 8`.
