...
`SHIFT + Cluster` or `SHIFT + Clip length` adjusts `Rate`, which scales the structural base grid before tuplets and ratchets. `1x` is the default beat-anchor grid. Slower rates spread required anchors across the phrase: in `4/4`, `1/2x` keeps beats 1 and 3, while `1/4x` keeps the first beat of each bar in a four-bar clip. Compound eighth-note meters such as `6/8`, `9/8`, and `12/8` use dotted-quarter structural anchors at `1x`; `3x` brings back eighth-level detail as optional material. Faster rates `2x`, `3x`, and `4x` compact the structure like reading the phrase faster: they add denser eighth/triplet/sixteenth-like structural cells, make tuplet spans shorter, and give ratchets shorter cells to split. Density can thin the added fast-rate cells because they are high-priority structure, not all immune anchors.
Lowering `Density` removes generated hits without stretching the retained notes, including when clustering is active; note lengths continue to follow the full-density rhythmic structure and are capped again after clustered timing compensation. The displayed range is `20%` to `100%`, and generated rhythm material uses that displayed percentage as its phrase budget: `20%` is the sparse minimum. Anchor-only patterns thin back to the pulse, but once tuplets or ratchets exist the minimum keeps compact generated phrase cells first, with anchors treated as supporting material. `100%` keeps the full generated structure. Density thinning prefers compact pairs and short gestures across active ratchet, tuplet, and nested-ratchet material, rather than letting one generated layer consume the whole density budget. `ALT + Density` toggles density direction: `Strong` keeps structurally central pulses first, while `Weak` keeps lower-rank decorative material first. Thinning is monotonic, so hits drop out in a stable order instead of disappearing and reappearing as you keep turning in one direction.
`Cluster` pushes retained hits toward a contiguous phrase region. With the default rhythm rotation it lands at the end of the clip; at `50%`, the retained phrase is constrained to the back half, and at `100%` it is constrained to the final quarter. Internally, rhythm rotation can move that region while preserving the retained hit count. `ALT + Cluster` adjusts clip play start. Clustered anchors and generated gesture leads are compensated onto the sixteenth-note grid when possible, falling back to a thirty-second-note approximation when crowded. Interior tuplet and ratchet hits keep their local proportions from the generated gesture when there is room; if that would create rapid-fire spacing, the crowded interiors are thinned instead. A final cleanup pass also removes the less useful hit from any pair that would still land closer than a thirty-second note, using a modest structural bias without always preferring maximum-velocity leads. Clustered phrases stay easier to align with the song while preserving more of the source rhythm's shape. When clustering is active, tuplets add material into the compressed phrase instead of clearing the covered span first. Density chooses from the same phrase identity at `0%` and clustered settings; clustering changes where the retained gesture lands, not which layer is treated as musically important. Velocity contour positions stay attached to the full generated structure.

Tuplet is usually a half-bar transform: `Tuplet` sets how many target spans are affected, while `Tup.Div` sets how many divisions each selected span is split into. In compound eighth-note meters, each tuplet target span follows the dotted-quarter structural beat instead. Controller feedback shows target count versus split count, for example `4 spans` means four target spans and `x5` means each selected span is divided into five. Tuplet and ratchet target order are both derived from the whole clip as a single phrase. Tuplets claim broad phrase spans. Ratchets use the default longer-note-biased target order, decorating less central material while avoiding a pile-up of tiny bursts too early; `ALT + User 2 / Encoder 4` toggles `Rat Mode` to `B`, which follows stricter Barlow-style low-rank-first targeting. `Target Phase` rotates through the relevant target order. Ratchet comes after tuplets: `Ratchet` sets how many parent regions are affected, while `Rat.Div` sets how many divisions each selected parent region is split into. Parent regions are the beat cells plus any tuplet cells created inside the selected spans, so `4 cells` means four parent regions are selected and `x4` means each selected region is split into four. Very small parent cells are skipped when the selected ratchet division would create sub-thirty-second-note rolls. Tuplet and ratchet divisions support every integer from `x2` through `x16`; dense counts remain available globally, but individual target regions can be skipped when they are too short to split cleanly.
