...
Lowering `Density` removes generated hits without stretching the retained notes; note lengths continue to follow the full-density rhythmic structure. Density thinning is monotonic, so hits drop out in a stable order instead of disappearing and reappearing as you keep turning in one direction.
`ALT + Density` controls clustering, pushing retained hits toward a contiguous phrase region at the end of the clip without changing the retained hit count. At `0%` cluster, anchors remain protected; as clustering rises, anchors join the density selection so the whole phrase can concentrate when there are other hits to trade against. It does not rotate note positions, and velocity contour positions stay attached to the full generated structure.
